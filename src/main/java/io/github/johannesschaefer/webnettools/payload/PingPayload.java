...
@Data
@Tool(name="ping", displayName="Ping", cmd="oping", description="Standard Linux Ping tooling.")
public class PingPayload implements Payload {
    @MainParameter(displayName ="IP / Hostname", description="ping ip")
    private String host;

    @NumberParam(displayName ="Count", param="-c", min=1., max=10000., step=1., description="Stop after sending (and receiving) count ECHO_RESPONSE packets. If this option is not specified, ping will operate until interrupted.  If this option is specified in conjunction with ping sweeps, each sweep will consist of count packets.")
